...
try:
    from .lecteur import VideoPlayer as LecteurVideo
except Exception:  # pragma: no cover - fallback for script execution
    try:
        from components.lecteur import VideoPlayer as LecteurVideo  # type: ignore
    except Exception:  # pragma: no cover - keep graceful degradation
        LecteurVideo = None  # type: ignore
...
class ApercuVideos(QWidget):
    def __init__(self, dossier_videos: str, parent=None):
        ...
        if LecteurVideo is None:
            raise RuntimeError("LecteurVideo component is not available.")
